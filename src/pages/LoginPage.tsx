...
import { Mail, Lock, Loader2, ShieldCheck, ChevronDown } from 'lucide-react';
...
const LoginPage: React.FC = () => {
    ...
    const [loginType, setLoginType] = useState<'magic' | 'password'>('password');
    ...
    const { user, signInWithEmail, signInWithEmailPassword, signInWithGoogle, updateRole } = useAuth();
        ...
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex items-center justify-center p-4">
            <div className="max-w-md w-full bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl p-10 border border-slate-100 dark:border-slate-800">
                <div className="text-center mb-10 flex flex-col items-center">
                    ...
                    <h1 className="text-3xl font-black text-slate-900 dark:text-white mb-2 tracking-tight">Access Portal</h1>
                    <p className="text-slate-500 dark:text-slate-400 font-medium text-sm">Secure Institutional Authentication</p>
                </div>
                ...
                <div className="space-y-6 mb-8">
                    ...
                    <div className="space-y-3">
                        <label className="block text-[10px] font-black uppercase tracking-[0.25em] text-slate-400 ml-1">
                        ...
                        <div className="flex bg-slate-100 dark:bg-slate-800 p-1.5 rounded-2xl border border-slate-200 dark:border-slate-700">
                            <button
                                ...
                                className={`flex-1 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${loginType === 'password' ? 'bg-white dark:bg-slate-700 text-[#1e3a6e] shadow-md scale-[1.02]' : 'text-slate-500 hover:text-slate-700'}`}
                            >
                                Password
                            </button>
                            <button
                                ...
                                className={`flex-1 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${loginType === 'magic' ? 'bg-white dark:bg-slate-700 text-[#1e3a6e] shadow-md scale-[1.02]' : 'text-slate-500 hover:text-slate-700'}`}
                            >
                                Magic Link
                            </button>
                        </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6" autoComplete="off">
                    ...
                    <button
                        ...
                        className="w-full flex items-center justify-center gap-3 py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-2xl font-black uppercase tracking-widest text-xs shadow-xl shadow-blue-500/20 active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed group"
                    >
                        {isSubmitting ? (
                            <>
                            ...
                            <>
                                <span>{loginType === 'magic' ? 'Send Magic Link' : 'Secure Authorization'}</span>
                ...
                <div className="mt-8 text-center space-y-4">
                    <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">
                        ...
                        <Link to="/signup" className="text-blue-600 dark:text-blue-400 font-black hover:underline underline-offset-4">
                            Sign Up
                        </Link>
                    </p>
                    <p className="text-[10px] font-black uppercase tracking-tighter text-slate-400 max-w-xs mx-auto">
                        Authorized personnel only. All access is logged and digitally notarized by the institution.
                    </p>
                </div>
            </div>
        </div>
